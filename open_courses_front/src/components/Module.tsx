import { ModuleWithMaterials, type Module } from "@/dataclasses/course"
...
const Module = ({ index, module }: ModuleProps) => {
        ...
        <Draggable draggableId={`module-${module.id}`} index={index}>
            {(provided) => (
                <div className="flex-row gap-5 p-5 rounded-md border  border-gray-400" ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                    {/* Элемент, за который можно перетаскивать */}
                            ...
                            <Droppable droppableId={`materials-${module.id}`} type="material">
                                {(provided) => (
                                    <div ref={provided.innerRef} {...provided.droppableProps} className="space-y-2">
                                        {module.materials ? module.materials.map((material, materialIndex) => (
                                            <Material
                                                key={material.id}
                                                material={material}
                                                index={materialIndex}
